  ...
  try {
    ...
    if (salaryRangeDiversity && Object.keys(salaryRangeDiversity).length > 0) {
      ...
      if (currency !== "" && currency !== undefined)
        updatedSalaryRange.currency = currency;
      if (frequency !== "" && frequency !== undefined)
        updatedSalaryRange.frequency = frequency;

      if (Object.keys(updatedSalaryRange).length > 0) {
        fieldsToUpdate.salaryRangeDiversity = {
          ...updatedSalaryRange,
          min:
            updatedSalaryRange.min ??
            (await Job.findById(jobId)).salaryRangeDiversity.min,
          max:
            updatedSalaryRange.max ??
            (await Job.findById(jobId)).salaryRangeDiversity.max,
          currency:
            updatedSalaryRange.currency ??
            (await Job.findById(jobId)).salaryRangeDiversity.currency,
          frequency:
            updatedSalaryRange.frequency ??
            (await Job.findById(jobId)).salaryRangeDiversity.frequency,
        };

        // Validate min < max
        if (
          fieldsToUpdate.salaryRangeDiversity.min >=
          fieldsToUpdate.salaryRangeDiversity.max
        ) {
          return res.status(400).json({
            message: "Minimum salary must be less than maximum salary.",
            success: false,
    ...
    if (workLocation && Object.keys(workLocation).length > 0) {
      ...
      if (state !== "" && state !== undefined)
        updatedWorkLocation.state = state;
      if (country !== "" && country !== undefined)
        updatedWorkLocation.country = country;

      if (Object.keys(updatedWorkLocation).length > 0) {
        fieldsToUpdate.workLocation = {
          ...updatedWorkLocation,
          city:
            updatedWorkLocation.city ??
            (await Job.findById(jobId)).workLocation.city,
          state:
            updatedWorkLocation.state ??
            (await Job.findById(jobId)).workLocation.state,
          country:
            updatedWorkLocation.country ??
            (await Job.findById(jobId)).workLocation.country,
        };

        // Backward compatibility for 'location' field
        fieldsToUpdate.location = `${fieldsToUpdate.workLocation.city || ""}, ${
          fieldsToUpdate.workLocation.state || ""
        }, ${fieldsToUpdate.workLocation.country}`;
      }
    }
    ...
    if (availabilityFrame && Object.keys(availabilityFrame).length > 0) {
      ...
      if (startDate !== "" && startDate !== undefined)
        updatedAvailabilityFrame.startDate = new Date(startDate);
      if (endDate !== "" && endDate !== undefined)
        updatedAvailabilityFrame.endDate = new Date(endDate);

      if (Object.keys(updatedAvailabilityFrame).length > 0) {
        fieldsToUpdate.availabilityFrame = {
          ...updatedAvailabilityFrame,
          startDate:
            updatedAvailabilityFrame.startDate ??
            (await Job.findById(jobId)).availabilityFrame.startDate,
          endDate:
            updatedAvailabilityFrame.endDate ??
            (await Job.findById(jobId)).availabilityFrame.endDate,
        };
      }
    }
...
//     // Salary range filte    r (if salary is stored as Number in DB)
