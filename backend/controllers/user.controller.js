...
const handleError = (res, error, defaultMessage = "Server Error") => {
  ...
  return res
    .status(500)
    .json({ message: defaultMessage, success: false, error: error.message });
};
...
export const register = async (req, res) => {
  try {
    ...
    const requiredFields = !isOtpVerified
      ? ["email", "firstname", "role", "gender"]
      : ["email", "firstname", "role", "gender", "phoneNumber", "password"];
    if (
      ...
      return res.status(400).json({
        message: `All ${requiredFields.join(", ")} are required`,
        success: false,
      });
    }
    ...
    if (!isVerifiedOtp) {
      ...
      const roleFeatures =
        role === "candidate"
          ? "browse jobs, apply directly, and track your applications in real-time"
    ...
    if (isVerifiedOtp) {
      if (!phoneNumber || !password) {
        return res.status(400).json({
          message:
            "Phone number and password are required to complete registration",
          success: false,
        });
      }
...
export const login = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message: "Incorrect email, password, role, or unverified email.",
        success: false,
      });
    }
...
export const requestOtp = async (req, res) => {
  try {
    ...
    await sendEmail(
      email,
      "Your OTP for Login/Reset Password",
      otp,
      templatePath
    );
