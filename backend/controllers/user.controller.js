...
export const register = async (req, res) => {
  try {
    ...
    if (file) {
      const fileKey = `uploads/${Date.now()}_profile${path.extname(
        file.originalname
      )}`;
      const uploadParams = {
...
export const updateProfile = async (req, res) => {
  try {
    ...
    if (file) {
      const fileKey = `${
        user.role === "candidate" ? "resumes" : "logos"
...
export const resendOTP = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "New OTP sent successfully", success: true });
  } catch (error) {
    handleError(res, error);
  }
};
