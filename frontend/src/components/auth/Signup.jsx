...
//     agreedToTerms: false,
//   });
//   console.log(input.jobRole, "check jobRole... ")
...
const Signup = () => {
  ...
  // const isValidRecruiterEmail = (email) => {
  //   if (!email || typeof email !== "string") returnpli false;
  //   const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  //   const trimmedEmail = email.trim().toLowerCase();
  //   if (!emailRegex.test(trimmedEmail)) return false;
  //   const [, domain] = trimmedEmail.split("@");
  ...
  const isValidRecruiterEmail = (email) => {
    ...
    if (input.role === "candidate") {
      ...
      if (publicDomains.includes(domain)) return false; // Added semicolon here
      const validDomainEndings = [".com", ".co", ".org", ".net", ".biz", ".in", ".tech", ".io", ".ai", ".info", ".edu"];
      return validDomainEndings.some((ending) => domain.endsWith(ending));
    }
    return false;
  ...
  const renderStep = () => {
    switch (input.step) {
      case 1:
        return (
          <div className="space-y-6">
            <h2 className="text-2xl sm:text-3xl font-bold text-center">Create a new account</h2>
      ...
      case 2:
        return (
          <form onSubmit={submitHandler} className="space-y-4">
            <h2 className="text-xl sm:text-2xl font-bold text-center">Sign up as {input.role}</h2>
            <div>
              <Label htmlFor="email">Email *</Label>
              <Input
                id="email"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="flex justify-center items-center min-h-[calc(100vh-64px)] px-4 py-6">
        <div className="w-full max-w-4xl flex flex-col md:flex-row bg-white shadow-2xl rounded-2xl overflow-hidden">
          ...
          <div className="hidden md:block md:w-1/2 bg-gradient-to-br from-yellow-400 to-orange-500 p-6 sm:p-8 flex flex-col justify-center items-center">
            ...
            />
            <div className="mt-6 text-center text-white">
              <h2 className="text-xl sm:text-2xl font-bold">Practice to Master Skills</h2>
              <div className="flex flex-wrap justify-center gap-4 mt-6">
                <div className="bg-white bg-opacity-30 p-3 rounded-lg shadow-md text-sm">5-Day Interview Prep</div>
                <div className="bg-white bg-opacity-30 p-3 rounded-lg shadow-md text-sm">Mock Interviews</div>
                <div className="bg-white bg-opacity-30 p-3 rounded-lg shadow-md text-sm">100-Day Coding Sprint</div>
              </div>
            </div>
          </div>
          {/* Form Section */}
          <div className="w-full md:w-1/2 p-6 sm:p-8 flex flex-col justify-center">
            <div className="space-y-6">
              ...
              {input.step === 1 && (
                <div className="flex justify-end mt-6">
                  <Button onClick={nextStep} disabled={loading || !input.role} className="w-full sm:w-auto">
                    {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Next"}
                  </Button>
                </div>
              )}
