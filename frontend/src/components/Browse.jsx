...
const Browse = () => {
  useGetAllJobs();
  const { allJobs, isLoading, searchedQuery } = useSelector((store) => store.job);
  const dispatch = useDispatch();
  const [location, setLocation] = useState(searchedQuery.location || '');

  const handleLocationChange = (value) => {
    setLocation(value);
    dispatch(setSearchedQuery({ keyword: searchedQuery.keyword, location: value }));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <div className="max-w-7xl mx-auto my-10 px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          <>
            <h1 className="font-bold text-2xl text-gray-800 mb-10">Discovering Jobs...</h1>
            <LoadingSkeleton />
          </>
        ) : allJobs.length > 0 ? (
